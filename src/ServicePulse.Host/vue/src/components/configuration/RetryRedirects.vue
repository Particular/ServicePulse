...
} from "../../composables/serviceRedirects";
...
function saveDeleteRedirect() {
  useDeleteRedirects(selectedRedirect.value.message_redirect_id).then(
    (result) => {
      if (result.message === "success") {
        redirectSaveSuccessful.value = true;
