...
function getArchiveGroups(classifier) {
  return useGetArchiveGroups(classifier).then((result) => {
    ...
    let maxIndex = archiveGroups.value.reduce(
      (currentMax, currentGroup) => Math.max(currentMax, currentGroup.index),
      0,
    );

    result.forEach((serverGroup) => {
      let previousGroup = archiveGroups.value.find(
        (oldGroup) => oldGroup.id == serverGroup.id,
      );
    ...
    archiveGroups.value = result
      .filter(
        (group) =>
          !undismissedRestoreGroups.value.find(
            (deletedGroup) => deletedGroup.id === group.id,
          ),
      )
...
function restoreGroup() {
  ...
  selectedGroup.value = archiveGroups.value.find(
  ...
  useRestoreGroup(group.id).then((result) => {
    if (result.message === "success") {
      ...
      useShowToast(
        "error",
        "Error",
        "Failed to restore the group:" + result.message,
      );
    }
  });
}

var statusesForRestoreOperation = [
  "restorestarted",
  "restoreprogressing",
  "restorefinalizing",
  "restorecompleted",
...
function isRestoreInProgress() {
  return archiveGroups.value.some(
    (group) =>
      group.workflow_state.status !== "none" &&
      group.workflow_state.status !== "restorecompleted",
  );
}
          ...
          <div class="messagegrouplist">
            <div ref="messageGroupList">
              <div class="row">
                <div class="col-sm-12">
                  <no-data
                    v-if="archiveGroups.length === 0 && !loadingData"
                        ...
                        <div class="row">
                          <div class="col-sm-12 no-side-padding">
                              ...
                              <div class="col-sm-12 no-side-padding">
                                ...
                                >
                                  <span class="metadata">
                                    ...
                                    >
                                    <span
                                      v-if="group.operation_remaining_count > 0"
                                    >
                                      (currently restoring
                                  ...
                                  <span class="metadata">
                                    ...
                                    <time-since
                                      :date-utc="
                                        group.last_operation_completion_time
                                      "
                                    ></time-since>
                                  </span>
                                </p>
                            ...
                            <div
                              class="row"
                              v-if="
                                !isBeingRestored(group.workflow_state.status)
                              "
                            >
                              <div class="col-sm-12 no-side-padding">
                                <button
                                    ...
                                    <ul class="retry-request-progress">
                                      ...
                                      >
                                        ...
                                        <button
                                          ...
                                          v-if="
                                            group.need_user_acknowledgement ==
                                            true
                                          "
                                    ...
                                    <div class="op-metadata">
                                      <span class="metadata"
                                        ><i
                                          ...
                                          class="fa fa-clock-o"
                                        ></i>
                                        Restore request started:<time-since
