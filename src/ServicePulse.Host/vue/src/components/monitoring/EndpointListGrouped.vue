...
<template>
  <section v-if="grouping.selectedGrouping > 0">
    ...
    <div class="row">
      <div class="col-12 no-side-padding">
        <div class="row" v-for="endpointGroup in grouping.groupedEndpoints" :key="endpointGroup">
          <div class="endpoint-group-title">
            {{ endpointGroup.group }}
          </div>
          <!-- <div class="row box endpoint-row" ng-repeat="endpointDescription in endpointGroup.endpoints | filter: filter.name" ng-init="endpoint = endpointDescription.endpoint" ng-mouseenter="endpoint.hover1=true" ng-mouseleave="endpoint.hover1=false"> -->
          <div class="row box endpoint-row" v-for="endpointDescription in endpointGroup.endpoints" :key="endpointDescription.shortName">
            <div class="col-12 no-side-padding">
              <div class="row">
                    ...
                    <div class="col-5 no-side-padding endpoint-status">
                      ...
                      <span class="warning" ng-if="endpoint.errorCount" uib-tooltip="{{endpointDescription.endpoint.errorCount | metricslargenumber}} failed messages associated with this endpoint. Click to see list.">
                        <a ng-if="endpoint.errorCount" class="warning btn" href="/#/failed-messages/group/{{endpoint.serviceControlId}}">
                          ...
                          <span class="badge badge-important ng-binding">{{ endpointDescription.endpoint.errorCount }}</span>
                        </a>
                      </span>
                    </div>
                  </div>
                ...
                <div class="col-xs-2 col-xl-1 no-side-padding">
                  <div class="row box-header">
                    <div class="no-side-padding">
                      <graph plot-data="endpoint.metrics.retries" avg-label-color="#CC1252" metric-suffix="MSGS/S" minimum-YAxis="{{smallGraphsMinimumYAxis.retries}}" class="graph retries pull-left"></graph>
                    </div>
                    <div class="no-side-padding sparkline-value">
