        ...
        <div class="col-sm-12">
          <div class="tabs">
            ...
            <h5
              ...
              :class="{
                active:
                  subIsActive('all-failed-messages') ||
                  subIsActiveSubPath('/failed-messages/group/'),
                disabled:
                  !connectionState.connected &&
                  !connectionState.connectedRecently,
              }"
            >
              <RouterLink :to="{ path: '/failed-messages/all-failed-messages' }"
                >All Failed Messages
              </RouterLink>
              <span
                v-if="stats.number_of_failed_messages !== 0"
            ...
            >
              <RouterLink
                :to="{ path: '/failed-messages/deleted-message-groups' }"
            ...
            <h5
              ...
              :class="{
                active:
                  subIsActive('all-deleted-messages') ||
                  subIsActiveSubPath('/deleted-messages/group/'),
                disabled:
                  !connectionState.connected &&
                  !connectionState.connectedRecently,
              }"
            >
              <RouterLink
                :to="{ path: '/failed-messages/all-deleted-messages' }"
                >All Deleted Messages
              </RouterLink>
              <span
                v-if="stats.number_of_archived_messages !== 0"
            ...
            <!--All Pending Retries -->
            <h5
              v-if="!licenseStatus.isExpired && showPendingRetry"
              :class="{
                active: subIsActive('pending-retries'),
                disabled:
                  !connectionState.connected &&
                  !connectionState.connectedRecently,
              }"
            >
              <RouterLink :to="{ path: '/failed-messages/pending-retries' }"
                >Pending Retries
              </RouterLink>
              <span
                v-if="stats.number_of_pending_retries !== 0"
                class="badge badge-important"
                >{{ stats.number_of_pending_retries }}</span
              >
            </h5>
          </div>
        </div>
