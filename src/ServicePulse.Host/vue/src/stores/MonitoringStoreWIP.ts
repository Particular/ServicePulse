...
export const useMonitoringStore = defineStore("MonitoringStore", () => {
  ...
  function getPropertyValue(obj: Endpoint, path: string): number {
    const properties = path.split(".");
    //TODO: this loses all type safety, since the intermediate steps return various objects and only the
    // final step returns a number. Is there a more idiomatic way of achieving this?
    return properties.reduce((accumulator, current) => accumulator[current] as any, obj as any);
  }
