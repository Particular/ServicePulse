...
config.plugins.push(function() {
    this.plugin('done', function (statsData) {
        ...
        if (!stats.errors.length) {
            ...
            const migrateHtml = fs.readFileSync('./app/migrate/index.html', 'utf8');

            tokenizedMarkup = migrateHtml.split('.js?v=').map(token => token.split('"></script>'));
            flattenedTokens = tokenizedMarkup.reduce((acc, val) => acc.concat(val), []);
            htmlOutput = flattenedTokens.filter((token, index) => index % 2 == 0).join('.js?v=' + now + '"></script>');

            fs.writeFileSync('./app/migrate/index.html', htmlOutput);
        }
    });
})
