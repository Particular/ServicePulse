...
describe("Feature: Backend mininum requirements checking", () => {
  describe("Rule: It should be validated that ServiceControl version is at least 5.0.0", () => {
    ...
    test("Example: ServiceControl main instance is 5.0.0", async ({driver}) => {
        await driver.setUp(precondition.serviceControlWithMonitoring);
        await driver.setUp(precondition.hasServiceControlVersion("5.4.0"));

        //Act
        await driver.goTo("usage");
        
        //Await for the version to be displayed - TODO: replace to use role and/or aria-lable
        expect(await screen.findByText(/v5\.4\.0/i)).toBeInTheDocument();
    ...
    test("Example: ServiceControl main instance is 4.9.0", async ({driver}) => {
        //Arrange
        await driver.setUp(precondition.serviceControlWithMonitoring);        
