...
import { parseTimePoint, resolveTimeRange, loadDefaultRange, saveDefaultRange, factoryDefaultRange, narrowingPresets, describeRangeText } from "@/components/audit/timeRange";
...
describe("RULE: A range reads as 'from to to'; a range that ends now is just its start", () => {
  test("EXAMPLE: A last-N range shows only the start expression", () => {
    expect(describeRangeText({ from: "now-90m", to: "now" })).toBe("now-90m");
  });

  test("EXAMPLE: An absolute start that runs to now keeps its 'to now', since the start alone would read as a point in time", () => {
    expect(describeRangeText({ from: "2026-09-01 08:00", to: "now" })).toBe("2026-09-01 08:00 to now");
  });

  test("EXAMPLE: Two live expressions are joined by 'to'", () => {
    expect(describeRangeText({ from: "now-1d/d", to: "now/d" })).toBe("now-1d/d to now/d");
  });

  test("EXAMPLE: Absolute bounds are joined by 'to'", () => {
    expect(describeRangeText({ from: "2026-09-01 08:00", to: "2026-09-01 12:00" })).toBe("2026-09-01 08:00 to 2026-09-01 12:00");
  });
});
